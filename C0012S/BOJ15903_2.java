...
public class BOJ15903_2 {
    ...
    static PriorityQueue<Long> a; // 처음 카드의 상태(카드에 쓰여진 수)를 저장하는 우선 순위 큐 (1 ≤ a_i ≤ 1,000,000)

    public static void cardUnionGame() { // 카드 합체 놀이 후, 만들 수 있는 가장 작은 점수를 계산하는 함수
        long sum;
        for (int u = 0; u < m; u++) { // 카드 합체 과정
            sum = a.poll() + a.poll(); // 고른 2 장의 카드에 쓰여진 수의 합
            a.offer(sum); // 2 장의 카드에 쓰여진 수의 합을 첫 번째 카드에 덮어 쓰기
            a.offer(sum); // 2 장의 카드에 쓰여진 수의 합을 두 번째 카드에 덮어 쓰기
        }

        long minScore = 0; // 카드 합체 놀이 후, 만들 수 있는 가장 작은 점수
        for (long cardScore : a) { // 카드 합체가 끝난 후, 놀이의 점수 계산
            minScore += cardScore;
        }
    ...
    public static void main(String[] args) throws IOException {
        ...
        a = new PriorityQueue<>();
        for (int i = 0; i < n; i++) {
            a.offer(Long.parseLong(token.nextToken()));
        }
