...
public class BOJ3184_2 {
    ...
    public static void findAnimalNum(int x, int y) { // 영역 안의 양의 수와 영역 안의 늑대의 수를 구하는 함수  // DFS Algorithm 적용
        for (int d = 0; d < 4; d++) { // 상, 하, 좌, 우 방향에 있는 다음 지점 체크
            int nx = x + dx[d]; // 다음 지점의 행의 인덱스
            int ny = y + dy[d]; // 다음 지점의 열의 인덱스
            ...
            if (check(nx, ny) && garden[nx][ny] != '#') { // 다음 지점이 마당의 범위를 벗어나지 않았고, 다음 지점에 울타리가 있지 않을 경우
                ...
                findAnimalNum(nx, ny); // 다음 지점의 주변 지점 검사
            }
        }
    }
        ...
        for (int r = 0; r < R; r++) {
            for (int c = 0; c < C; c++) {
                if (garden[r][c] != '#') { // 울타리가 아닐 경우
                    ...
                    if (garden[r][c] == 'o') { // 검사를 시작할 지점의 좌표에 양이 있을 경우
                        sheepNum += 1; // 영역 안의 양의 수 추가
                    }
                    else if (garden[r][c] == 'v') { // 검사를 시작할 지점의 좌표에 늑대가 있을 경우
                        wolfNum += 1; // 영역 안의 늑대의 수 추가
                    }
                    garden[r][c] = '#'; // 검사를 시작할 지점의 좌표의 마당의 구조를 울타리로 변경
