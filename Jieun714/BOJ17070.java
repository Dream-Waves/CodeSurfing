...
public class BOJ17070 {
    ...
    public static void bfs(int y, int x){
        ...
        while(!que.isEmpty()) {
            ...
            if(direction == 0 || direction == 2) { //가로이거나 대각선일때
                if(isVisited(ny, nx+1)) //방문 가능하다면
                    que.add(new int[] {ny, nx+1, 0});
            }
            if(direction == 1 || direction == 2) { //세로이거나 대각선일때
                if(isVisited(ny+1, nx)) //방문 가능하다면
                    que.add(new int[] {ny+1, nx, 1});
            }
            if(isCrossCheck(ny, nx)){ //대각선 파이프가 차지할 수 있을 때
