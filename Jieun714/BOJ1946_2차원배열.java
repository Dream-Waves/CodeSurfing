...
public class BOJ1946_2차원배열 {
    ...
    public static void checkMax(){
        max = 0; //선발할 수 있는 신입사원의 최대 인원수
        Arrays.sort(arr, Comparator.comparingInt((int[] o) -> o[0])); //2차원 배열 정렬
        int now = arr[0][1]; //비교할 면접 성적
        for(int j=1; j<N; j++){
            if(now > arr[j][1]) { //수가 적을 수록 높은 순위
                max++; //개수 증가
                now = arr[j][1]; //비교 값 변경
            }
        }
    ...
    public static void main(String[] args) throws IOException {
        ...
        for(int i=0; i<T; i++){
            N = Integer.parseInt(br.readLine()); //지원자의 개수
            arr = new int[N][2]; //서류심사 성적, 면접 성적의 순위를 담는 배열
            for(int j=0; j<N; j++){
