 ...
 * 해결: 그리디 알고리즘. 최대 K개의 집중국의 수신 가능 영역의 길이의 합의 최솟값을 구하기 위해 정렬 사용
 * */
...
public class BOJ2212 {
    public static void main(String[] args) throws IOException {
        ...
        for(int i=0; i<list.size()-1; i++) {
            newList.add(list.get(i+1)-list.get(i)); //센서 좌표의 차이를 리스트에 추가
        }
        Collections.sort(newList); //오름차순 정렬
        int sum = 0; //최대 K개의 집중국의 수신 가능 영역의 길이의 합
        for(int i=0; i<=newList.size()-K; i++) {
            sum += newList.get(i);
        }
