 ...
 *      "[닉네임]님이 들어왔습니다."  채팅방에서 누군가 나가면 다음 메시지가 출력된다.
...
public class PRO42888 {
    public String[] solution(String[] record) {
        ...
        for(String str : record){
            String [] arr = str.split(" "); //[액션] [유저 아이디] [닉네임]
            switch(arr[0]){
                case "Enter": //입장
                    map.put(arr[1], arr[2]);
                    que.add(new String [] {arr[1], "님이 들어왔습니다."});
                    break;
                case "Leave": //퇴실
                    que.add(new String [] {arr[1], "님이 나갔습니다."});
                    break;
                case "Change": //닉네임 변경
                    map.put(arr[1], arr[2]); //HashMap의 경우, 이미 키값이 존재한다면 매핑되는 값이 변경됨
                    break;
            }
        }
        ...
        while(!que.isEmpty()){ //큐가 빌때까지
            String [] now = que.poll();
            answer[idx] = map.get(now[0]) + now[1]; //정답배열에 최종닉네임으로 변경 값을 담음
            idx++;
        }
        return answer;
    }
