...
public class BOJ1946_1차원배열 {
    public static void main(String[] args) throws IOException {
        ...
        for(int i=0; i<T; i++){
            int N = Integer.parseInt(br.readLine()); //지원자의 개수
            int [] arr = new int[N]; //서류심사 성적, 면접 성적의 순위를 담는 배열
            for(int j=0; j<N; j++){
            ...
            int max = 0;
            int now = arr[0]; //비교할 면접 성적
            for(int j=1; j<N; j++){
                if(now > arr[j]) { //수가 적을 수록 높은 순위
                    max++; //개수 증가
                    now = arr[j]; //비교 값 변경
                }
            }
